...
import { Calendar, History, MailCheck, MailX } from 'lucide-react';
...
export default async function NewsletterHistoryPage() {
  ...
  const { data: sends, error } = await supabase
    ...
    .select(
      'id, status, created_at, started_at, completed_at, total_recipients, sent_count, failed_count, newsletters(subject)'
    )
                    ...
                    <tr key={s.id} className="border-b last:border-b-0 hover:bg-muted/30 transition-colors">
                      ...
                      <td className="py-3 px-2">
                        <div className="flex items-center gap-2 text-sm text-muted-foreground">
                          ...
                          {new Date(s.started_at ?? s.created_at).toLocaleString()}
                        </div>
                      </td>
                      <td className="py-3 px-2">
